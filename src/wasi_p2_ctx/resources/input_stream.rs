...
static RESOURCE_TYPE: LazyLock<ResourceType> = LazyLock::new(|| {
    ResourceType::new::<WasiP2InputStreamResource>(Some(TypeIdentifier::new(
...
impl ResourceConvert for WasiP2InputStreamResource {
    fn ty_own() -> ValueType {
    ...
    fn from_value(ctx: impl AsContext, value: Value) -> anyhow::Result<Self> {
        match value {
            ...
            _ => anyhow::bail!(
                "Expected resource of type WasiP2InputStreamResource, but got {value:?} instead"
            ),
        }
    }
