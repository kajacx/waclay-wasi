...
#[derive(Debug, Clone)]
pub struct WasiP2DescriptorResource {}

static RESOURCE_TYPE: LazyLock<ResourceType> = LazyLock::new(|| {
    ResourceType::new::<WasiP2DescriptorResource>(Some(TypeIdentifier::new(
        "WasiP2DescriptorResource",
        None,
    )))
});

impl WasiP2DescriptorResource {
    pub fn resource_type() -> ResourceType {
        RESOURCE_TYPE.clone()
    }
}

impl ComponentType for WasiP2DescriptorResource {
    fn ty() -> ValueType {
    ...
    fn from_value(value: &Value, ctx: impl AsContext) -> anyhow::Result<Self> {
        match value {
            ...
            _ => anyhow::bail!(
                "Expected resource of type WasiP2DescriptorResource, but got {value:?} instead"
            ),
        }
    }
